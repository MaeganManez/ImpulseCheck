...
async function sendOTPEmail(toEmail, otp, type) {
  type = type || 'register';
  var subject = type === 'register' ? 'ImpulseCheck - Verify Your Account' : 'ImpulseCheck - Password Reset OTP';
  var heading = type === 'register' ? 'Verify Your Account' : 'Reset Your Password';
  var bodyText = type === 'register' ? 'Thank you for signing up! Use the OTP below to verify your account.' : 'We received a request to reset your password. Use the OTP below.';
  var html = '<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px 20px;">'
  ...
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || 'ImpulseCheck <noreply@impulsecheck.com>',
    to: toEmail,
    subject: subject,
    html: html,
  });
...
async function sendReportEmail(toEmail, userName, reportData, period) {
  var sym = reportData.currency || 'PHP ';
  var rows = reportData.purchases.map(function(p) {
    return '<tr>'
  ...
  var html = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;">'
    + '<h1 style="color:#1a4a5c;">ImpulseCheck</h1>'
    ...
    + '<tr style="background:#2eaa7e;color:#fff;"><th style="padding:8px;text-align:left;">Item</th><th style="padding:8px;text-align:left;">Category</th><th style="padding:8px;text-align:right;">Price</th><th style="padding:8px;text-align:center;">Decision</th></tr>'
    + rows
    + '</table></div>';
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || 'ImpulseCheck <noreply@impulsecheck.com>',
...
module.exports = { sendOTPEmail: sendOTPEmail, sendReportEmail: sendReportEmail };
